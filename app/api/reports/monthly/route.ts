...
export async function GET(req: Request) {
  ...
  const vehicles = allVehicles.filter(
    (v) =>
      v.route === session.user.route &&
      (!vehicleFilter || v.vehicleId === vehicleFilter),
  );
  const students = allStudents.filter(
    (s) =>
      s.route === session.user.route &&
      (!studentFilter || s.studentId === studentFilter),
  );
  ...
  for (const date of dateKeys) {
    const bookings = allBookings.filter(
      (b) => b.travelDate === date && b.bookingStatus === "Booked",
    );
    ...
    const studentAgg = new Map(
      students.map((s) => [
        s.studentId,
        { booked: 0, present: 0, absent: 0, clubbed: 0 },
      ]),
    );
    const vehicleNotSentRows: {
      date: string;
      vehicleId: string;
      affected: number;
      reason: string;
    ...
    for (const date of dateKeys) {
      ...
      for (const v of views) {
        ...
        if (
          v.operationalVehicleId &&
          v.operationalVehicleId !== v.defaultVehicleId
        ) {
          totalClubbed++;
        ...
        if (v.operationalVehicleId) {
          ...
          if (vAgg) {
            ...
            if (
              v.attendance === "Absent - Student" ||
              v.attendance === "Absent - Vehicle Not Sent"
            )
              vAgg.absent++;
          }
        }
      ...
      for (const status of statuses) {
        ...
        if (status.status === "Not Sent") {
          ...
          const affected = views.filter(
            (v) =>
              v.operationalVehicleId === status.vehicleId &&
              v.attendance === "Absent - Vehicle Not Sent",
          ).length;
          vehicleNotSentRows.push({
            date,
    ...
    const clubbingRows = allClubbing.filter(
      (c) =>
        ...
        c.date <= end &&
        (!vehicleFilter ||
          c.toVehicleId === vehicleFilter ||
          c.fromVehicleId === vehicleFilter) &&
        (!studentFilter || c.studentId === studentFilter),
    );
    ...
    doc
      .fontSize(18)
      .text("COLLEGE TRANSPORT ATTENDANCE REPORT", { align: "center" });
    doc.moveDown(0.5);
  ...
  function drawTableHeader(doc: PDFKit.PDFDocument, cols: string[]) {
    ...
    cols.forEach((c, i) => {
      doc
        .font("Helvetica-Bold")
        .text(c, 40 + i * colWidth, y, { width: colWidth });
    });
    doc.moveDown(0.5);
    doc.font("Helvetica");
  }
